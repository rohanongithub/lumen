...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Mark page as loaded after a brief delay to ensure smooth transitions
  ...
  return (
    <main className="min-h-screen">
      {/* Show onboarding before any other content */}
      {showOnboarding && <Onboarding onClose={handleOnboardingClose} />}
      
      {/* Only show page content after it's loaded */}
                    ...
                    <div className="w-full">
                      <div className="flex flex-col gap-6 relative z-20">
                        {freshOutAlbums.filter(album => !album.hidden).map((album) => (
                          ...
                          >
                            <div className="relative w-full h-auto sm:h-[250px] md:h-[300px] flex flex-col sm:flex-row">
                              <div className="relative w-full sm:w-2/5 md:w-1/2 h-[200px] sm:h-full">
                                <Image 
                                  src={album.image} 
                                  alt={album.title}
                                  fill
                              ...
                              <div className="w-full sm:w-3/5 md:w-1/2 bg-transparent backdrop-blur-sm flex flex-col justify-center p-4 sm:p-5 md:p-8">
                                <h3 className="text-xl sm:text-2xl font-bold mb-1 text-white">{album.title}</h3>
                                <p className="text-base sm:text-lg text-white/90 mb-1">{album.artist}</p>
                                <p className="text-xs sm:text-sm text-white/70 mb-2">{album.year}</p>
                                <p className="text-sm sm:text-base text-white/80 max-w-xl line-clamp-3 sm:line-clamp-none">{album.description}</p>
